...
Actor* FabricaNivel1::crearInstanciaTronco(float _x, float _y)
{
	Actor* actor = new Tronco();
	actor->setAvatar(avatarTroncoN2);
	actor->setX(_x);
	actor->setY(_y);
	actor->setGameManager(getGameManager());
	if (getGameManager()->moverActorA(actor, _x, _y) == false)
...
Actor* FabricaNivel1::crearInstanciaCharco(float _x, float _y)
{
	Actor* actor = new Charco();
	actor->setAvatar(avatarCharcoN2);
	actor->setX(_x);
	actor->setY(_y);
	actor->setGameManager(getGameManager());
	if (getGameManager()->moverActorA(actor, _x, _y) == false)
...
Actor* FabricaNivel1::crearInstanciaRoca(float _x, float _y)
{
	Actor* actor = new Roca();
	actor->setAvatar(avatarRocaN2);
	actor->setX(_x);
	actor->setY(_y);
	actor->setGameManager(getGameManager());
	if (getGameManager()->moverActorA(actor, _x, _y) == false)
...
Actor* FabricaNivel1::crearInstanciaBase(float _x, float _y)
{
...
Actor* FabricaNivel1::crearInstanciaAguila(float _x, float _y)
{
	return nullptr;
}
	...
	for (int f = 0; f < getMapaNivel().size(); f++) {
		for (int c = 0; c < getMapaNivel()[f].size(); c++) {
			...
			case celdaSimbolo_Charco: {
				Charco* tronco = (Charco*)crearInstanciaCharco(c, f);
				break;
			}
			case celdaSimbolo_Roca: {
				Roca* roca = (Roca*)crearInstanciaRoca(c, f);
				break;
			}
			}
		}
...
Actor* FabricaNivel1::crearInstanciaArbusto(float _x, float _y) {
	...
	actor->setAvatar(avatarArbustoN3);
	actor->setX(_x);
	actor->setY(_y);
	actor->setGameManager(getGameManager());
	if (getGameManager()->moverActorA(actor, _x, _y) == false)
